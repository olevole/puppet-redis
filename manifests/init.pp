...
class redis-new( $ensure=present,
	$version="latest",
	$overcommit=1,
	$redisname="redis",
	$bind="0.0.0.0",
	...
	case $operatingsystem {
		centos, redhat, debian, ubuntu: {
			...
			$dir="/var/lib/redis"
			Sysctl::Conf["vm.overcommit_memory"] -> Class ["redis-new::install"] -> File["$redisconfig"] -> Service["$servicename"]
	...
	class { "redis-new::install": ensure => $ensure, version => $version }

	file { "$redisconfig":
		notify  => $notify_restart ? {
			true    => Exec["redis-server_restart"],
			default => undef,
		},
		mode => '0444',
		ensure  => present,
